...
/*   Updated: 2026/05/18 19:20:30 by yben-dje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*list_to_flat_array(t_list *list, void *(*memcpy_func)(void *,
			const void *, size_t), size_t (*size_func)(const void *))
{
	size_t	size;
	t_u32	index;
	char	*dest;
	...
	index = 0;
	dest = mem_alloc(size + 1, NULL, NULL);
	if (!dest)
		return (NULL);
	cursor = 0;
	while (index < list->size)
...
void	*list_as_array(t_list *list)
{
	...
	array = mem_alloc((list->size + 1) * sizeof(void **), NULL, NULL);
	if (!array)
		return (NULL);
	iterator = iterator_new(list, 0);
	index = 0;
	while (index < list->size)
		array[index++] = iterator_next(&iterator);
	array[index] = 0;
	return (array);
}
