...
/*   Updated: 2026/06/01 14:13:15 by lchamard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	update_pwd(t_minishell *minishell)
{
	...
	if (!env_variables_set(&minishell->env_variables_manager, "PWD",
			path_buffer))
		return (false);
	return (true);
}
...
bool	set_old_pwd(t_minishell *minishell)
{
	...
	if (old_pwd)
	{
		if (!env_variables_set(&minishell->env_variables_manager, "OLDPWD",
				old_pwd))
			return (false);
	}
	else
