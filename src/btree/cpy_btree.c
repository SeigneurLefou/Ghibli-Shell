...
/*   Updated: 2026/06/01 14:06:46 by lchamard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	cpy_btree_node(t_btree_node *new, t_btree_node *old)
{
	...
	if (old->left)
	{
		new->left = mem_alloc(sizeof(t_btree_node), default_error_exit, NULL,
				0b1);
	...
	if (old->right)
	{
		new->right = mem_alloc(sizeof(t_btree_node), default_error_exit, NULL,
				0b1);
...
bool	cpy_btree(t_btree *new, t_btree *old)
{
	...
	new->expr = old->expr;
	new->node = mem_alloc(sizeof(t_btree_node), default_error_exit, NULL, 0b1);
	cpy_node = old->node;
