...
/*   Updated: 2026/03/26 09:40:49 by lchamard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_io_fds(t_btree_node node, int *fds[2], t_vec *expr)
{
	...
	while (i < node->io_files->size)
	{
		...
		if (io_file->type == io_type_infile)
			open_file(vec_get(expr, node->file_name_token_index), O_RDONLY,
				&(*fds)[0]);
		else if (io_file->type == io_type_infile)
			(*fds)[0] = here_doc_file(vec_get(expr,
						node->file_name_token_index));
		else if (io_file->type == io_type_outfile)
			open_file(vec_get(expr, node->file_name_token_index), O_CREAT
