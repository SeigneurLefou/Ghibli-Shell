/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   syntax_checker_parentheses.c                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yben-dje <yben-dje@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/03 17:03:32 by yben-dje          #+#    #+#             */
/*   Updated: 2026/06/03 17:04:10 by yben-dje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "token.h"

static void	check_matching_parentheses_logic(t_token *token, int *par_index,
		int *count, unsigned int index)
{
	if (token->type == token_type_scope_delimiter && token->data.data[0] == '(')
	{
		if (*par_index < 0)
			*par_index = index;
		(*count)++;
	}
	if (token->type == token_type_scope_delimiter && token->data.data[0] == ')')
		(*count)--;
}

t_parsing_check_result	check_matching_parentheses(t_vec *expr)
{
	int				count;
	t_token			*token;
	unsigned int	index;
	int				par_index;

	count = 0;
	index = 0;
	par_index = -1;
	while (index < expr->size)
	{
		token = (t_token *)vec_get(expr, index);
		check_matching_parentheses_logic(token, &par_index, &count, index);
		if (!count)
			par_index = -1;
		if (count < 0)
			return ((t_parsing_check_result){parse_err_unmatching_parentheses,
				index, -1});
		index++;
	}
	if (count)
		return ((t_parsing_check_result){parse_err_unmatching_parentheses,
			par_index, index});
	return ((t_parsing_check_result){parse_err_success, 0, 0});
}

unsigned int	get_matching_parentheses_left(t_vec *expr, unsigned int index)
{
	int		count;
	t_token	*token;

	count = 1;
	index++;
	while (index < expr->size && count)
	{
		token = (t_token *)vec_get(expr, index);
		if (token->type == token_type_scope_delimiter
			&& token->data.data[0] == '(')
			count++;
		if (token->type == token_type_scope_delimiter
			&& token->data.data[0] == ')')
			count--;
		index++;
	}
	return (index);
}

t_parsing_check_result	check_unsuported_arithmetic(t_vec *expr)
{
	unsigned int	index;
	t_token			*token1;
	t_token			*token2;
	unsigned int	matching1;
	unsigned int	matching2;

	index = 0;
	while (index < expr->size)
	{
		token1 = (t_token *)vec_get(expr, index);
		token2 = (t_token *)vec_get(expr, index + 1);
		if (token1->type == token_type_scope_delimiter
			&& token1->data.data[0] == '('
			&& token2->type == token_type_scope_delimiter
			&& token2->data.data[0] == '(')
		{
			matching1 = get_matching_parentheses_left(expr, index);
			matching2 = get_matching_parentheses_left(expr, index + 1);
			if (matching2 + 1 == matching1)
				return ((t_parsing_check_result){
					parse_err_unsupported_arithmetic, index, matching1 - 1});
		}
		index++;
	}
	return ((t_parsing_check_result){parse_err_success, 0, 0});
}

t_parsing_check_result	check_empty_parentheses(t_vec *expr)
{
	unsigned int	index;
	t_token			*token;

	index = 0;
	while (index < expr->size)
	{
		token = (t_token *)vec_get(expr, index);
		if (token->type == token_type_scope_delimiter
			&& token->data.data[0] == '(')
		{
			token = (t_token *)vec_get(expr, index + 1);
			if (token->type == token_type_scope_delimiter
				&& token->data.data[0] == ')')
				return ((t_parsing_check_result){parse_err_empty_parentheses,
					index, index + 1});
		}
		index++;
	}
	return ((t_parsing_check_result){parse_err_success, 0, 0});
}
